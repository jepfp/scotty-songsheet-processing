...
private class LiedSourcePdfFileFinder(implicit db: Db) {
  ...
  protected def performQuery(liedId: Long): Seq[LiedWithData] = {
    ...
    val result: Seq[LiedWithData] = Await.result(dbReadFuture, Duration.Inf).map(x => LiedWithData(SourceSystem.Scotty, x._1, x._2, x._3, List.empty[String], x._4.map(_.toLocalDateTime).getOrElse(LocalDateTime.MIN), x._5.map(_.toLocalDateTime).getOrElse(LocalDateTime.MIN), x._6, FileType.Pdf(), None))
    result
  }
}