...
import ch.scotty.job.json.{JobConfiguration, JobDefinitions, SingleSongToImageConverterJobConfiguration}
...
class JobRunnerTest extends UnitSpec {
  ...
  val jobDefinitions: JobDefinitions = JobDefinitions(Some(Seq(SingleSongToImageConverterJobConfiguration(uuid, 5))), None)
  ...
  "runAllJobs" should "return one success job result for a defined job" in {
    //arrange
    val testee = new JobRunner(jobDefinitions, singleSongToImageConverterJobStub, allSongToImageConverterJobStub)
    //act
  ...
  "runAllJobs" should "create a map entry with a key named after the class of the executed job" in {
    ...
    val testee = new JobRunner(jobDefinitions, concreteJob, allSongToImageConverterJobStub)
    //act
    val jobResults = testee.runAllJobs()
    //assert
